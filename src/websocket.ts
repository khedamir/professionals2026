...
interface WsWithContext extends WebSocket {
  context?: WsClientContext;
}

export function createBoardWebSocketServer(server: http.Server) {
  const wss = new WebSocketServer({ server, path: '/ws/board' });

  wss.on('connection', async (ws: WsWithContext, req) => {
    ...
    if (boardId) {
      ...
      const hasAccess = board.accessList.some(
        (a) => a.userId === ctx.userId && a.canEdit,
      );
      if (!hasAccess) {
      ...
      const initial: BoardOutgoingEvent = {
        type: 'full_state',
        boardId: board.id,
    ...
    ws.on('message', async (data) => {
      ...
      const board =
        (ctx.boardId && (await store.findBoardById(ctx.boardId))) ||
        (ctx.boardHash && (await store.findBoardByHash(ctx.boardHash))) ||
        undefined;
      if (!board) return;
      ...
      let outgoing: BoardOutgoingEvent | null = null;

      switch (event.type) {
        case 'focus_object': {
          if (!ctx.userId || !ctx.userName) return;
          const lock = await store.lockObject(board.id, event.objectId, {
            id: ctx.userId,
            email: '',
            name: ctx.userName,
            passwordHash: '',
        ...
        case 'blur_object': {
          ...
          const ok = await store.unlockObject(
            board.id,
            event.objectId,
            ctx.userId,
          );
        ...
        case 'add_object': {
          if (!ctx.userId) return;
          await store.upsertObject(board.id, event.object);
          outgoing = {
            type: event.type,
            object: event.object,
          } as BoardOutgoingEvent;
          break;
        }
        case 'delete_object': {
          if (!ctx.userId) return;
          await store.deleteObject(board.id, event.objectId);
          outgoing = {
            type: 'delete_object',
            objectId: event.objectId,
          };
      ...
      if (outgoing) {
        ...
        wss.clients.forEach((client) => {
          const c = client as WsWithContext;
          if (
